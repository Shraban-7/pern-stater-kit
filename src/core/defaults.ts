import type { PresetId, StarterConfig } from './types.js';

export const DEFAULT_NODE_VERSION = '20';

export function createDefaultConfig(name: string): StarterConfig {
  return {
    version: 1,
    name,
    language: 'typescript',
...
export function applyPreset(config: StarterConfig, preset: PresetId): StarterConfig {
  ...
  switch (preset) {
    ...
    case 'api':
      ...
      break;
    case 'saas':
      next.architecture = 'modular-monolith';
      next.architectures = ['modular-monolith', 'monorepo'];
      next.cache = 'redis';
      next.queue = 'bullmq';
